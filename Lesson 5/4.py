...
def sumSubValue(sign):
    number = userText.get(1.0,tk.END).strip()
    if len(number)!=0:
        global var
        var=var+sign*int(number)
...
addButton = tk.Button(main_window, text="Прибавить", command=lambda: sumSubValue(1),width=12,height=3)
addButton.place(relx=0.40, rely=0.30, anchor='center')

addButton = tk.Button(main_window, text="Вычесть", command=lambda: sumSubValue(-1),width=12,height=3)
